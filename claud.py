...
llm = OpenAI(
    ...
    model_name="gpt-3.5-turbo-instruct"  # Use faster model
)
...
FIXCONNECT_KEYWORDS = [
    'fixconnect', 'engineer', 'service', 'electrical', 'hvac', 
    'plumbing', 'security', 'power', 'quote', 'booking',
    'payment', 'stripe', 'register', 'login', 'password',
    'profile', 'chat', 'message', 'technical', 'issue',
    'repair', 'maintenance', 'installation'
]

def is_fixconnect_related(question: str) -> bool:
    """Check if question is related to FixConnect app"""
    ...
    # Check service-related patterns
    service_patterns = [
        r'\b(need|want|looking for).*(help|service|engineer)\b',
        r'\b(electrical|plumbing|hvac).*(issue|problem|repair)\b',
        r'\b(how to|how do i|can i).*(register|login|book|pay)\b'
    ]
    ...
    try:
        ...
        if docs:
            ...
            try:
                ...
                # Return exact FAQ answer for high similarity
                if similarity_score > 0.7:
                    return {"answer": content.get("answer", doc.page_content.strip())}
                    
                # Otherwise, generate focused response from knowledge base
                template = """
                You are a FixConnect customer service assistant. Provide SHORT, FOCUSED answers using ONLY the FixConnect knowledge base.
                ...
                Instructions:
                1. Keep response SHORT (1-2 sentences)
                2. Use ONLY FixConnect knowledge base information
                3. Be specific about relevant FixConnect services or features
                4. Do not provide generic advice
                5. If service-related, mention how to request through FixConnect
        ...
        # If no FAQ match, use knowledge base directly
        template = """
        You are a FixConnect customer service assistant. Provide SHORT, FOCUSED answers using ONLY the FixConnect knowledge base.
        ...
        Instructions:
        1. Keep response SHORT (1 sentence)
        2. Use ONLY FixConnect knowledge base information
        3. Be specific about how to use FixConnect features
        4. Do not provide generic advice
